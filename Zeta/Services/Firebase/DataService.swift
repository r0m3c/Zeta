...
// Handle and uploading data, other than users from our Database
import Foundation
import SwiftUI
    ...
    func uploadPost(image: UIImage, caption: String?, displayName: String, userID: String, handler: @escaping (_ success: Bool) -> ()) {
        ...
        ImageManager.instance.uploadPostImage(postID: postID, image: image) { (success) in
            if success {
                ...
                print("Error uploading post image to Database")
                handler(false)
                return
            }
        }
        ...
        if let snapshot = querySnapshot, snapshot.documents.count > 0 {
            ...
            for document in snapshot.documents {
                ...
                if
                    ...
                    let displayName = document.get(DatabasePostField.displayName) as? String, let timeStamp = document.get(DatabasePostField.dateCreated) as? Timestamp {
